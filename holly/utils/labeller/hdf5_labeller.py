...
from holly.utils.datatypes import Pathlike, TensorComplex64, TensorFloat32
from holly.experiment.setup import Setup
from holly.experiment.probe import BeamConfig
...
class HDF5Labeller(Labeller):
    ...
    def update_dataset_size(self, new_size: int) -> None:
        if self.initialized:
            print(
                "WARNING: Cannot change the dataset size anymore. The HDF5 file was already prepared."
            )
            return
    ...
    def _initialize_hdf5_file(self) -> None:
        ...
        if self.store_probe:
            ...
            self._cached_probes = torch.zeros(
                (self.cache_size, *dim), dtype=torch.float32
            )
        ...
        if self.store_setup:
            self.setup_keys = (
                experiment.GLOBAL_EXP_SETUP.as_dict().keys()
            )  # just need the field names/ dict keys
            ...
            self._cached_setups = {
                key: torch.zeros(self.cache_size, dtype=torch.float32)
                for key in self.setup_keys
            }
            self.hdf5_ds_setups = {
                key: self.hdf5_setup.create_dataset(key, (self.num_samples,))
                for key in self.setup_keys
            }
        ...
        if self.store_polynomial:
            ...
            self.hdf5_ds_beam_cfg = {
                key: self.hdf5_beam_cfg.create_dataset(key, (self.num_samples,))
                for key in self.polynomial_keys
            }
    ...
    def flush(self) -> None:
        ...
        if self._store_hologram:
            self.hdf5_ds_holograms[lower:upper, ...] = self._cached_holograms[
                : self.cache_pointer, ...
            ]

        if self._store_gt_hologram:
            self.hdf5_ds_gt_holograms[lower:upper, ...] = self._cached_gt_holograms[
                : self.cache_pointer, ...
            ]

        if self._store_phantom:
            self.hdf5_ds_phantoms[lower:upper, ...] = self._cached_phantoms[
                : self.cache_pointer, ...
            ]

        if self._store_probe:
            self.hdf5_ds_probes[lower:upper, ...] = self._cached_probes[
                : self.cache_pointer, ...
            ]

        if self._store_flatfield:
            self.hdf5_ds_flatfields[lower:upper, ...] = self._cached_flatfields[
                : self.cache_pointer, ...
            ]

        if self.num_conditions > 0:
            self.hdf5_ds_conditions[lower:upper, ...] = self._cached_conditions[
                : self.cache_pointer, ...
            ]

        if self._store_polynomial:
            for key in self.polynomial_keys:
                self.hdf5_ds_beam_cfg[key][lower:upper] = self._cached_beam_configs[
                    key
                ][: self.cache_pointer]

        if self._store_setup:
            for key in self.setup_keys:
                self.hdf5_ds_setups[key][lower:upper] = self._cached_setups[key][
                    : self.cache_pointer
                ]
    ...
    def _cache_beam_cfg(self, beam_cfg: BeamConfig) -> None:
        if self._store_polynomial:
            for key in self.polynomial_keys:
                self._cached_beam_configs[key][
                    self._cache_pointer
                ] = beam_cfg.as_dict()[key]
    ...
    def output_pointer(self, val: int) -> None:
        if not isinstance(val, int):
            raise AttributeError(
                f"Output pointer must be of type int, not: {type(val)}"
            )
