...
# local libraries
from holly.utils.datatypes import Tensorlike
...
class FlexTransform(ABC):
    ...
    def transform(self, t: callable, *imgs) -> Tensorlike | Tuple[Tensorlike, ...]:
        ...
        assert (
            len(imgs) >= 1
        ), f"Must have at least 1 image in `imgs`, but got {len(imgs)}."
...
class RandomCropAndResize(FlexTransform):
    ...
    def __call__(self, *imgs: Tuple[Tensor, ...]) -> Tuple[Tensor, ...]:
        ...
        params = T.RandomCrop.get_params(
            imgs[0], output_size=(output_size, output_size)
        )
    ...
    def __call__(self, *imgs: Tuple[Tensor, ...]) -> Tuple[Tensorlike, ...]:
        def expand_dim(img: Tensorlike) -> Tensorlike:
            match t := type(img):
                ...
                case _:
                    raise ValueError(
                        f"Expecting either np.ndarray or torch.Tensor, but got: {t}"
                    )
...
class AdjustBrightness(FlexTransform):
    ...
    def __call__(self, *imgs: Tuple[Tensor, ...]) -> Tuple[Tensor, ...]:
        ...
        """
        # scale by 2*16-1 (UInt16 max value), since adjust brightness handles int-values weirdly and sets the maximum
        # value to one
        brightness_factor = np.random.uniform(self.lower, self.upper)
        return self.transform(
            lambda a: F.adjust_brightness(
                a / 65535, brightness_factor=brightness_factor
            )
            * 65535,
            *imgs,
        )
...
class AdjustContrast(FlexTransform):
    ...
    def __call__(self, *imgs: Tuple[Tensor, ...]) -> Tuple[Tensor, ...]:
        ...
        return self.transform(
            lambda a: F.adjust_contrast(a / 65535, contrast_factor=contrast_factor)
            * 65535,
            *imgs,
        )
...
class RemoveOutliers(FlexTransform):
    ...
    def get_transform(self) -> Callable[[Tensorlike], Tensor]:
        def transform(img: Tensorlike) -> Tensor:
            ...
            """
            img = (
                torch.tensor(img) if not isinstance(img, torch.Tensor) else img.clone()
            )
            assert img.ndim == 2
            filtered_img = self.median_pool(img[None, None, :, :])[0, 0, :, :]
            diff_img = img - filtered_img
            std_dev_value = torch.std(diff_img)
            pixels_to_correct = torch.where(
                abs(diff_img) > (self.threshold * std_dev_value)
            )
