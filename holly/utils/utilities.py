...
# local libraries
from holly.utils.datatypes import Range, Pathlike
from holly.utils import fileIO
from holly.utils.unit_conversions import convert_z01, convert_z02
...
def calc_Fr(energy: int, z01: float, z02: float, det_px_size: int) -> float:
    ...
    Args:
        ...
        z01 (float): The distance from the OSA to the object (in [cm] = 1e7 nm).
        z01 (float): Distance from the object to the detector (in [m] = 1e9 nm).
        det_px_size (int): Pixel size of detector (in [nm]).

    Returns:
        float: Fresnel number.
    """
    lam = 1.2398 / energy  # h*c / E[keV]
...
def randint_from_range(
    r: Range, size: int | None = None, incl_high: bool = True
) -> np.ndarray | int:
