...
# local libraries
import holly.utils.datatypes as dtypes
...
def read_csv(file: dtypes.Pathlike, header: bool = True) -> pd.DataFrame:
...
def load_single_line(
    path: dtypes.Pathlike, idx: int, header: Dict[str, Any] | bool
) -> List[int | str | float]:
    """Load a single line based on an index from csv-file.

    Args:
        path (dtypes.Pathlike): Path to either the csv-file.
    ...
    has_header = (
        ...
        else all([i != v for i, v in enumerate(header.values())])
    )
...
def load_img(
    path: dtypes.Pathlike, expand_dim: bool = False, convert_to_float32: bool = True
) -> np.ndarray:
    img = io.imread(path)
...
def save_img(img: np.ndarray, path: dtypes.Pathlike) -> None:
    path = Path(path)
...
def load_hologram(path: dtypes.Pathlike) -> np.ndarray:
    return load_img(path)


def save_hologram(
    path: dtypes.Pathlike,
    holo: dtypes.Hologram,
    idx: int,
    format: str,
    fill_zeros: int = 6,
) -> str:
    ...
    Args:
        path (dtypes.Pathlike): Outputfolder, where to save the hologram
        holo (dtypes.Hologram): Hologram will be saved.
...
def save_phantom(
    path: dtypes.Pathlike, phantom: Phantom | None, idx: int, fill_zeros: int = 6
) -> str | None:
    """Save Phantom as a pickle file.

    Args:
        path (dtypes.Pathlike): Outputfolder, where to save the phantom.
...
def load_phantom(path: dtypes.Pathlike) -> Phantom:
...
def read_json(path: dtypes.Pathlike) -> Dict[str, Any]:
    """Load data from a JSON file.

    Args:
        path (dtypes.Pathlike): The path to the JSON file.
...
def write_json(data: Dict[str, Any], path: dtypes.Pathlike) -> None:
