...
from holly.experiment.probe.beam_config import get_random_beam_config
from holly.experiment.probe import Probe, PolynomialProbe
...
class ProbeGenerator:
    ...
    def __init__(self, config: ConfigParser) -> None:
        ...
        if config.get("probe_noise", None) is not None:
            self.probe_noise: module_noise.Noise = config.init_obj(
                "probe_noise", module_noise, size=self.probe_size
            )
        else:
