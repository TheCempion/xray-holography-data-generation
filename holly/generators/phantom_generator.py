...
from holly.objects.physical_properties import PhysicalProperties
from holly.objects.placement import ObjectPlacement
import holly.objects.shape_sampler as module_sampler
import holly.utils.random as random
from holly.utils.torch_settings import get_torch_device
from holly.utils.utilities import randint_from_range
...
class PhantomGenerator:
    def __init__(self, config: ConfigParser) -> None:
        ...
        self.shape_sampler: module_sampler.ShapeSampler = config.init_obj(
            "shape_sampler", module_sampler
        )
    ...
    def create_phantom(self) -> Phantom:
        ...
        for _ in range(num_objects):
            ...
            physical_object = (
                phys_props.phaseshift + 1j * phys_props.absorption
            ) * shape.shape
            composed_phantom += Phantom(physical_object, phantom_size, center)
        composed_phantom.set_limits_absorption(
            min=self.absorption_min, max=self.absorption_max
        )
        composed_phantom.set_limits_phaseshift(
            min=self.phaseshift_min, max=self.phaseshift_max
        )
