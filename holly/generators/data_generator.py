...
# local libraries
import holly.utils.labeller as module_labeller
import holly.experiment as experiment
from holly.utils.utilities import crop_center
...
class DataGenerator:
    ...
    def generate_data(self, num_samples: int = None) -> None:
        ...
        for idx in tqdm(range(self.num_samples)):
            ...
            holo = self.hologram_generator.create_hologram(
                phantom=phantom, probe=probe, flatfield=flatfield
            )
            raw_holo = self.hologram_generator.gt_hologram
            cropped_probe = (
                crop_center(probe.probe, holo.shape)
                if self.cfg_data_generator["crop_probe"]
                else probe.probe
            )
            ...
            self.labeller.cache(
                ...
                flatfield=(
                    flatfield.flatfield.cpu() if flatfield is not None else None
                ),
                condition=(
                    flatfield.condition.cpu()
                    if flatfield is not None and flatfield.condition is not None
                    else None
                ),
                beam_cfg=beam_cfg,
                setup=setup,
    ...
    def _get_labeller(
        self, num_samples: int, dataset_name: str, override: bool
    ) -> module_labeller.Labeller:
        detector_size = experiment.GLOBAL_EXP_SETUP.detector_size
        probe_target_size = (
            detector_size
            if self.cfg_data_generator["crop_probe"]
            else experiment.GLOBAL_EXP_SETUP.probe_size
        )
