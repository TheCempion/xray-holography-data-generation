...
class ObjectPlacement:  # TODO: naming does not make any sense if it only returns the center pos
    ...
    def place_centered(self, obj: torch.Tensor) -> Tuple[int, int]:
        ...
        pos_x, pos_y = np.random.normal(
            mu, sigma
        )  # 3*sigma interval to have more "accepted" objects in boundaries
    ...
    @staticmethod
    def object_fits(
        obj: torch.Tensor, ambient_size: int, center: Tuple[int, int] | None
    ) -> bool:  # TODO: incl. thr
    ...
    @staticmethod
    def embedd(
        ambient_size: int, obj: torch.Tensor, center: Tuple[int, int]
    ) -> torch.Tensor:
        cx, cy = center
        h, w = obj.shape
        start_x = cx - h // 2
        ...
        ambient = torch.zeros(
            (ambient_size, ambient_size), dtype=obj.dtype, device=get_torch_device()
        )
