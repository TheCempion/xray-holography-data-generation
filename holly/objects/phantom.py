...
# local libraries
from holly.utils.datatypes import Pathlike, TensorFloat32, TensorComplex64
...
class Phantom:
    def __init__(
        self, phys_obj: TensorComplex64, size: int, center: Tuple[int, int] = None
    ) -> None:
        self.size = size
    ...
    def _embedd(
        self, physical_object: TensorComplex64, new_size: int, center: Tuple[int, int]
    ) -> TensorComplex64:
        ...
        if ObjectPlacement.object_fits(
            obj=physical_object, ambient_size=new_size, center=center
        ):
            return ObjectPlacement.embedd(
                obj=physical_object, ambient_size=new_size, center=center
            )
        else:
            return torch.zeros(
                (new_size, new_size), dtype=torch.cfloat, device=get_torch_device()
            )

    def pad_to(self, new_size: int) -> TensorComplex64:
        if new_size < self.size:
            raise ValueError(
                f"New size ({new_size}) must be greater or equal the phantom`s size ({self.size})."
            )

        # return self._embedd(self.phantom, new_size=new_size, center=None)
        padding = int((new_size - self.size) / 2)
        padded = torch.nn.functional.pad(
            self.phantom, pad=[padding] * 4, mode="constant", value=0
        )
        return padded
    ...
    def show(
        self, phaseshift: bool = True, absorption: bool = True, suptitle: str = ""
    ) -> None:
    ...
    def _create_figure(
        self, phaseshift: bool, absorption: bool, suptitle: str = "Phantom"
    ) -> None:
        plt.close("all")
        plt.ioff()
    ...
    def phantom(self, value: TensorComplex64) -> None:
        if not (self.size, self.size) == value.shape:
            raise AttributeError(
                f"Shapes are not the same. Is: {value.shape}). Should be {self._phantom.shape}."
            )
        self._phantom = value
    ...
    def phaseshift(self, value: Any) -> None:
        raise AttributeError(
            "Cannot set 'phaseshift' property directly. Modify 'phantom' instead."
        )
    ...
    def absorption(self, value: Any) -> None:
        raise AttributeError(
            "Cannot set 'absorption' property directly. Modify 'phantom' instead."
        )
    ...
    def real(self, value: Any) -> None:
        raise AttributeError(
            "Cannot set 'real' property directly. Modify 'phantom' instead."
        )
    ...
    def imag(self, value: Any) -> None:
        raise AttributeError(
            "Cannot set 'imag' property directly. Modify 'phantom' instead."
        )
