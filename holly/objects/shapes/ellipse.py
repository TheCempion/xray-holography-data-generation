...
class Ellipse(Shape):
    def __init__(
        self, radius_r: int, radius_c: int, cuts: List[float] = [0] * 4
    ) -> None:
        """TODO
        ...
        shape = torch.zeros(
            (2 * radius_r, 2 * radius_c), device=get_torch_device(), dtype=torch.float
        )
    ...
    def _cut_mask(self, cut: float, side: str, vec: np.array, size: int) -> np.array:
        if side.lower() in "lt":  # left or top
            ...
            raise ValueError(
                f"Invalid side. Must be one of [l]eft, [r]ight, [t]op or [b]ottom. Actual: {side}."
            )
