...
from holly.utils.datatypes import Pathlike, Tensorlike, TensorFloat32
from holly.utils import fileIO
from holly.experiment.flatfield import BaseFlatField
...
class MultiChannelFlatField(BaseFlatField):
    def __init__(
        self, flatfield: Tensorlike | Pathlike, conditions: List[Tensorlike | Pathlike]
    ) -> None:
    ...
    def condition(self, val: TensorFloat32) -> None:
        ...
        if not isinstance(val, TensorFloat32):
            raise AttributeError(
                f"Invalid datatype for a flatfield stack: {type(val)}]"
            )

        if not val.ndim == 3:
            raise AttributeError(
                f"Invalid number of dimensions. Should be 3, but it {val.ndim}]"
            )
