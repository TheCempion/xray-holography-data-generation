...
from holly.utils.noise import Noise
...
class PolynomialProbe(Probe):
    def __init__(
        self, beam_cfg: BeamConfig, size: int, center_beam: bool, noise: Noise
    ) -> None:
    ...
    def _create_probe(
        ...
        def add_probe_component(
            coeff: float, *, pow_x: int, pow_y: int
        ) -> TensorFloat32:
            if coeff == 0:
                return 0
