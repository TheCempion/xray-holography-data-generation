...
from holly.utils.datatypes import TensorComplex64, TensorFloat32
from holly.objects.phantom import Phantom
from holly.experiment.flatfield.flatfield import FlatField

from .simulation import Simulation
from holly.experiment.probe import Probe
from holly.experiment.setup import NFHSetup
...
class NFHSimulation(Simulation):
    ...
    def forward(
        self, phantom: Phantom, probe: Probe, flatfield: FlatField | None = None
    ) -> TensorFloat32:
    ...
    def _make_hologram(
        self, psi_det: TensorComplex64, flatfield: FlatField = None
    ) -> TensorFloat32:
    ...
    def gt_hologram(self, val: TensorFloat32) -> None:
        if not isinstance(val, TensorFloat32):
            raise AttributeError(
                f"hologram must be of type `TensorFloat32`, not {type(val)}"
            )
