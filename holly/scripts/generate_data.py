...
# local libraries
from holly.generators import (
    DataGenerator,
    HologramGenerator,
    ProbeGenerator,
    PhantomGenerator,
)
import holly.generators as module_generators
from holly.parse_config import ConfigParser
import holly.utils.torch_settings as torch_settings
...
def main():
    ...
    parser.add_argument(
        "num_samples", type=int, help="Number of data samples that should be generated."
    )
    parser.add_argument(
        "output", type=str, help="Output folder where the generated data is stored."
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    if config.get("flatfield_generator", None) is None:
        flatfield_generator = module_generators.DummyFlatFieldGenerator(config)
        config["labeller"]["args"]["store_flatfield"] = False
    else:
        flatfield_generator = config.init_obj(
            "flatfield_generator", module_generators, config=config
        )
